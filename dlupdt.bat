...
:: Check krd version
.\Tools\curl -# --etag-compare .\Tools\xml_tag.txt --etag-save .\Tools\xml_tag.txt -O "https://rescuedisk.s.kaspersky-labs.com/updatable/2018/bases/krd.xml" --ca-native > nul 2>&1
...
echo:
.\Tools\curl -# --etag-compare .\Tools\bases_tag.txt --etag-save .\Tools\bases_tag.txt -O "https://rescuedisk.s.kaspersky-labs.com/updatable/2018/bases/042-freshbases.srm" --ca-native
.\Tools\curl -# --etag-compare .\Tools\hashes_tag.txt --etag-save .\Tools\hashes_tag.txt -O "https://rescuedisk.s.kaspersky-labs.com/updatable/2018/bases/hashes.txt" --ca-native

echo:
echo Renaming files!
ren "042-freshbases.srm" "005-bases.srm" 2> nul
for /f "tokens=1" %%i in ('FINDSTR /L "042-freshbases.srm" "hashes.txt"') do echo %%i *005-bases.srm> 005-bases.srm.sha512 2> nul
echo:
echo:
...
echo:
FOR /F delims^=^"^ tokens^=2 %%G IN ('FINDSTR /L "databases_timestamp" "krd.xml"')  DO ECHO %%G > .\kavrescue\krd_bases_timestamp.txt & echo: & echo Bases timestamp is: %%G
...
.\Tools\curl -# -O "https://rescuedisk.s.kaspersky-labs.com/updatable/2018/krd.iso" --ca-native
echo:
...
echo Version on the disk: %isover%%isopatch% ][ Version on the server: %xmlver%%xmlpatch%
goto :input
) ELSE ( 
goto :continue
)
